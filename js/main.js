...
(() => {
  ...
  function startNewSession() {
    ...
    // ДИАГНОСТИКА: смотрим, какие стратегии у ботов
    console.log('[SESSION] Стратегии ботов:', botStrategies.join(', '));
  ...
  function startHand() {
    ...
    console.log('[HAND] Раздача #' + session.state.handNumber,
      '| BTN:', session.state.players[session.state.buttonIndex].name,
      '| банк после блайндов:', session.state.pot,
      '| currentBet:', session.state.currentBet,
      '| minRaise:', session.state.minRaise,
      '| actingIndex:', session.state.actingIndex,
      '| acting:', session.state.players[session.state.actingIndex].name);
  ...
  function botTurn(bot, idx) {
    ...
    setTimeout(() => {
      ...
      let action = Bots.decide(ctx);
      const toCall = Math.max(0, state.currentBet - bot.committedThisStreet);

      // === ДИАГНОСТИКА ===
      console.log('[BOT]', bot.name, '|', bot.strategy, '|', bot.position,
        '| рука:', ctx.card1.rank + ctx.card2.rank,
        '| suited:', ctx.card1.suit === ctx.card2.suit,
        '| улица:', ctx.street,
        '| toCall:', toCall,
        '| банк:', ctx.pot,
        '| facingRaise:', ctx.facingRaise,
        '| decision:', action.action, action.amount);
      // === /ДИАГНОСТИКА ===

      if (action.action === 'check' && toCall > 0) action = { action: 'fold', amount: 0 };
      ...
      if (!res.ok) {
        console.warn('[BOT] applyAction failed:', res.error, 'action:', action);
