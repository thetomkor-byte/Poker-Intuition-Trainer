/* ============================================================
   ...
   Чистый Vanilla JS. Всё в глобальном объекте Engine.
   ============================================================ */
...
const Engine = (() => {
  ...
  function evaluate5(cards) {
    ...
    if (isFlush && straightHigh) {
      if (straightHigh === 14) {
        return { category: HAND_CATEGORIES.ROYAL_FLUSH, ranks: [14], cards, name: 'Роял-флеш' };
      }
      return { category: HAND_CATEGORIES.STRAIGHT_FLUSH, ranks: [straightHigh], cards, name: 'Стрит-флеш' };
    }
    if (groups[0][0] === 4) {
      return { category: HAND_CATEGORIES.QUADS, ranks: [groups[0][1], groups[1][1]], cards, name: 'Каре' };
    }
    if (groups[0][0] === 3 && groups[1] && groups[1][0] === 2) {
      return { category: HAND_CATEGORIES.FULL_HOUSE, ranks: [groups[0][1], groups[1][1]], cards, name: 'Фулл-хаус' };
    }
    if (isFlush) {
      return { category: HAND_CATEGORIES.FLUSH, ranks: values, cards, name: 'Флеш' };
    }
    if (straightHigh) {
      return { category: HAND_CATEGORIES.STRAIGHT, ranks: [straightHigh], cards, name: 'Стрит' };
    }
    if (groups[0][0] === 3) {
      return { category: HAND_CATEGORIES.TRIPS, ranks: [groups[0][1], ...groups.slice(1).map(g => g[1])], cards, name: 'Сет' };
    }
    if (groups[0][0] === 2 && groups[1] && groups[1][0] === 2) {
      return { category: HAND_CATEGORIES.TWO_PAIR, ranks: [groups[0][1], groups[1][1], groups[2][1]], cards, name: 'Две пары' };
    }
    if (groups[0][0] === 2) {
      return { category: HAND_CATEGORIES.PAIR, ranks: [groups[0][1], ...groups.slice(1).map(g => g[1])], cards, name: 'Пара' };
    }
  ...
  function getBlindLevel(levelIndex) {
    const idx = Math.min(levelIndex, BLIND_LEVELS.length - 1);
    return BLIND_LEVELS[idx];
  }

  /* ==========================================================
     7. СОЗДАНИЕ ИГРОВОГО СОСТОЯНИЯ
     ========================================================== */
  ...
  function createInitialState(playerConfigs) {
    const players = playerConfigs.map((cfg, i) => ({
      ...
      stack: cfg.stack ?? STARTING_STACK,
      holeCards: [],
      folded: false,
      allIn: false,
      eliminated: false,
  ...
  function dealBoard(state, count) {
    ...
    for (let i = 0; i < count; i++) {
      state.board.push(state.deck[state._deckIdx++]);
    }
  }
  ...
  function preflopOrder(state) {
    ...
    const order = [];
    for (let i = 0; i < n; i++) {
      const idx = (state.buttonIndex + 3 + i) % n;
      order.push(idx);
    }
    return order;
  }
  ...
  function postflopOrder(state) {
    ...
    const order = [];
    for (let i = 0; i < n; i++) {
      const idx = (state.buttonIndex + 1 + i) % n;
      order.push(idx);
    }
    return order;
  }
  ...
  function postBlinds(state) {
    ...
    if (!sbPlayer.eliminated) {
      ...
      sbPlayer.stack -= pay;
      sbPlayer.committedThisStreet += pay;
      sbPlayer.committedTotal += pay;
      state.pot += pay;
    ...
    if (!bbPlayer.eliminated) {
      ...
      bbPlayer.stack -= pay;
      bbPlayer.committedThisStreet += pay;
      bbPlayer.committedTotal += pay;
      state.pot += pay;
  ...
  function applyAction(state, playerIndex, action) {
    ...
    if (!p || p.folded || p.allIn || p.eliminated) {
      return { ok: false, error: 'Игрок не может действовать' };
    }
    ...
    switch (action.type) {
      ...
      case 'raise': {
        ...
        if (target <= state.currentBet) {
          return { ok: false, error: 'Рейз должен быть больше текущей ставки' };
        }
        ...
        if (raiseSize < state.minRaise) {
          if (need < p.stack) {
            return { ok: false, error: 'Рейз меньше минимального' };
          }
        }

        p.stack -= need;
        p.committedThisStreet += need;
        p.committedTotal += need;
        state.pot += need;
        ...
        for (const other of state.players) {
          if (other !== p && !other.folded && !other.eliminated && !other.allIn) {
  ...
  /* ==========================================================
     14. ВСКРЫТИЕ И ОПРЕДЕЛЕНИЕ ПОБЕДИТЕЛЯ
     ========================================================== */
  ...
  function showdown(state) {
    ...
    for (const pot of pots) {
      ...
      for (const id of eligible) {
        ...
        if (!bestScore || compareScores(sc, bestScore) > 0) {
          bestScore = sc;
          winners = [id];
        } else if (compareScores(sc, bestScore) === 0) {
          winners.push(id);
        }
      }
    ...
    state.showdownResults = {
      scores: Object.fromEntries(
        [...scores.entries()].map(([id, sc]) => [id, sc ? { category: sc.category, name: sc.name, ranks: sc.ranks, cards: sc.cards } : null])
      ),
      pots: potResults,
      payouts,
      topWinnerId
  ...
  function awardToLastStanding(state) {
    ...
    state.showdownResults = {
      scores: {},
      pots: [{ amount: state.pot, winners: [winner.name], winnersIds: [winner.id] }],
      payouts: { [winner.id]: state.pot },
      topWinnerId: winner.id,
      uncontested: true
    };
    state.handOver = true;
    return state.showdownResults;
  ...
  function startHand(state) {
    ...
    let firstIdx = -1;
    for (const idx of order) {
      if (canAct(state.players[idx])) { firstIdx = idx; break; }
    }
  ...
  function getActionablePlayers(state) {
    return state.players.filter(p => canAct(p) && !p.folded);
  }
  ...
  return {
    ...
    HAND_CATEGORIES, CATEGORY_NAMES,
    BLIND_LEVELS, STARTING_STACK, SEATS,
    POSITIONS_9MAX, STREETS,
    ...
    dealHoleCards, burnCard, dealBoard,
    assignPositions, preflopOrder, postflopOrder,
    postBlinds,
    ...
    showdown, awardToLastStanding,

    getActivePlayers, getActionablePlayers, isHandOver,
