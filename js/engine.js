...
(() => {
  ...
  function botDecisionToEngineAction(decision) {
    return {
      type: decision.action,   // 'fold' | 'check' | 'call' | 'raise' | 'allin'
  ...
  function botTurn(bot, idx) {
    ...
    setTimeout(() => {
      ...
      console.log('[BOT]', bot.name, '|', bot.strategy, '|', bot.position,
        ...
        '| decision:', decision.action, decision.amount);

      // Конвертируем в формат движка
      let engineAction = botDecisionToEngineAction(decision);

      // Валидация: нельзя чекнуть при toCall > 0
      ...
      if (!res.ok) {
        console.warn('[BOT] applyAction failed:', res.error, 'action:', engineAction);
